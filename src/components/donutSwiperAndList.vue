...
<!-- init="false"  -->
<template >
  <swiper-container ref="swiperElChart" auto-height="true" observer="true" observe-slide-children="true"
    grab-cursor="true" pagination-dynamic-bullets="true" @activeindexchange="onActiveIndexChangeBySwiper">
    ...
    <swiper-slide v-for="range in props.dateRanges" :key="range">
      <donut-chart :date-range="range" :toggle-value="props.toggleValue" :percentage-threshold="percentageThreshold"
        @click:segment="donutSegmentClicked" class="q-pa-lg" style="min-height: 40px;" />
    </swiper-slide>
  </swiper-container>
    ...
    <div
      ...
      <q-list class="q-mt-xs">
        <transition-group appear enter-active-class="meala" leave-active-class="meala la" move-class="meala"
          ...
          <q-item
            v-for="item in momentsStore.aggregateData[props.dateRanges[props.activeIndex]][props.toggleValue].filter(item => displayOnlyOneNeed ? item.needName == displayOnlyOneNeed : item).filter(item => item[props.toggleValue == 'satisfaction' ? 'satisfactionImpactLabelValue' : (props.toggleValue == 'unsatisfaction' ? 'unsatisfactionImpactLabelValue' : 'importanceValue')] > percentageThreshold)"
...
<script setup>
import { watch, ref, onActivated, nextTick } from 'vue'
